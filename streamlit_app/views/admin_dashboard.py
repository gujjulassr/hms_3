    ...
    with tab6:
        ...
        with col_r:
            if st.button("Refresh", key="refresh_admin_chat"):
                st.session_state.pop("admin_chat_history", None)
                st.rerun()
        with col_clear:
        ...
        if "admin_chat_history" not in st.session_state:
            ...
            if hr.status_code == 200:
                st.session_state.admin_chat_history = hr.json()["messages"]
            else:
                st.session_state.admin_chat_history = []
        ...
        if chat_msg:
            ...
            st.session_state.admin_chat_history.append({"role": "user", "text": chat_msg})
            with st.chat_message("assistant"):
                ...
                st.write(reply)
            st.session_state.admin_chat_history.append({"role": "assistant", "text": reply})
